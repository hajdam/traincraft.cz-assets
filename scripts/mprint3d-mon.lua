...
if #args < 1 then
  io.write("Usage: mprint3d FILE [count]\n")
  os.exit(0)
end
local count = 1
if #args > 1 then
...
local primaryPrinter = component.printer3d
...
for address in component.list("printer3d") do
  io.write("Printer: " .. address .. "\n")
  local printer = component.proxy(address)
...
function printOut() 
  for address in component.list("printer3d") do
	...
	if not result then
      print("Error printing file: "..reason)
	  break
	end
  end
  ...
  term.setCursor(x, y)
  count = count - 1
  term.write("Printing progress. Remains: "..count.." ")
end

event.timer(5, printOut, count)
io.write("Printing started...")
